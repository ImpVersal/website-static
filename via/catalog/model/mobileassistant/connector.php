    ...
    public function getOrderProducts($data) {
        ...
        if($query->num_rows) {
            foreach($query->rows as $row) {
                ...
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$data['order_id'] . "' AND order_product_id = '" . (int)$row['order_product_id'] . "'");
                ...
                foreach ($order_product_options as $option) {
                    ...
                    $option_data[] = array($option['name'] => (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value));
                }
                ...
                if (isset($row['image']) && strlen($row['image']) > 0) {
                    $row['product_image'] = $this->model_tool_image->resize($row['image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
                } else {
                    $row['product_image'] = $this->model_tool_image->resize('placeholder.png', $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
                }
                unset($row['image']);
